...
from didian_duqu_xuanze import ImageSelectionDialog
import os

def read_from_library(location_row):
    """
    从资料库读取同名地点图片。
    逻辑：
    ...
    2. 如果没有选中项，则搜索资料库中所有同名图片：
       ...
       - 多个：弹出选择窗口。
    """
    name = location_row.name_edit.toPlainText().strip()
    if not name:
        QMessageBox.warning(location_row, "提示", "请先填写地点名称")
        return
    ...
    if found_path:
        location_row.set_image(found_path)
        return
    ...
    assets = store.list_assets("scene")  # 修改为 "scene" 以搜索地点/场景
    ...
    if not candidates:
        QMessageBox.information(location_row, "提示", f"资料库中未找到名为“{name}”的地点图片")
        return
        
    if len(candidates) == 1:
        location_row.set_image(candidates[0])
        print(f"[ReadLibrary] Auto-set single match: {candidates[0]}")
    else:
        # 多个匹配，弹出选择框
        dlg = ImageSelectionDialog(location_row, name, candidates)
        if dlg.exec():
            if dlg.selected_path:
                location_row.set_image(dlg.selected_path)
