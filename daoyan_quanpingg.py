...
class DirectorNode:
    ...
    def create_node(CanvasNode):
        ...
        class DraggableVideoWidget(QWidget):
            def __init__(self, video_paths, current_path=None, parent=None):
                super().__init__(parent)
                # 兼容单个路径或路径列表
                if isinstance(video_paths, str):
                    self.video_paths = [video_paths]
                elif isinstance(video_paths, list):
                    self.video_paths = [p for p in video_paths if p] # 过滤空值
                else:
                    self.video_paths = []
                
                # 确定当前显示的视频索引
                self.current_index = 0
                if current_path and current_path in self.video_paths:
                    self.current_index = self.video_paths.index(current_path)
            ...
            def setup_ui(self):
                ...
                if len(self.video_paths) > 1:
                    ...
                    self.btn_prev.setFixedSize(20, 30)
                    self.btn_prev.setStyleSheet("background-color: transparent; color: #666; border: none; font-weight: bold;")
                ...
                self.btn.clicked.connect(self.play_current_video)
                ...
                if len(self.video_paths) > 1:
                    ...
                    self.btn_next.setFixedSize(20, 30)
                    self.btn_next.setStyleSheet("background-color: transparent; color: #666; border: none; font-weight: bold;")
                    self.btn_next.clicked.connect(self.next_video)
                    layout.addWidget(self.btn_next)
                
                # Drag handle label
                self.drag_label = DragLabel("✊", self, self.get_current_video_path()) 
                self.drag_label.setToolTip(f"视频 ({self.current_index + 1}/{len(self.video_paths)})" if len(self.video_paths) > 1 else "视频")
            ...
            def play_current_video(self):
                path = self.get_current_video_path()
            ...
            def update_ui_state(self):
                path = self.get_current_video_path()
                # 更新 DragLabel 的视频路径
                self.drag_label.video_path = path
                self.drag_label.setToolTip(f"视频 ({self.current_index + 1}/{len(self.video_paths)})")
            ...
            #     # Use text/uri-list for file paths
            #     url = QUrl.fromLocalFile(self.video_path)
            #     mime_data.setUrls([url])
            #     # Also set text for compatibility
            #     mime_data.setText(self.video_path)
                
            #     drag.setMimeData(mime_data)
                
            #     # Visual feedback
            #     pixmap = QPixmap(100, 30)
            #     pixmap.fill(QColor("#E040FB"))
            #     painter = QPainter(pixmap)
            #     painter.setPen(Qt.white)
            #     painter.drawText(pixmap.rect(), Qt.AlignCenter, "拖拽视频")
            #     painter.end()
        ...
        class DirectorNodeImpl(CanvasNode):
            ...
            def __init__(self, x, y):
                # 更新计数器
                DirectorNodeImpl.instance_count += 1
                self.node_id = DirectorNodeImpl.instance_count
                
                # 节点初始化: x, y, width, height, title, icon
                # Icon can be None or a custom SVG path
                # 添加 ID 到标题
                super().__init__(x, y, 500, 400, f"导演节点 #{self.node_id}", None)
            ...
            def load_image_prompts(self):
                """加载图片提示词缓存"""
                try:
                    path = os.path.join(os.getcwd(), 'JSON', f'daoyan_image_prompts_{self.node_id}.json')
                    if os.path.exists(path):
                        with open(path, 'r', encoding='utf-8') as f:
                            return json.load(f)
                except Exception as e:
            ...
            def save_image_prompts(self):
                ...
                try:
                    dir_path = os.path.join(os.getcwd(), 'JSON')
                    os.makedirs(dir_path, exist_ok=True)
                    path = os.path.join(dir_path, f'daoyan_image_prompts_{self.node_id}.json')
                    with open(path, 'w', encoding='utf-8') as f:
            ...
            def load_character_paths(self):
                """加载人物图片路径缓存"""
                try:
                    path = os.path.join(os.getcwd(), 'JSON', f'daoyan_character_paths_{self.node_id}.json')
                    if os.path.exists(path):
                        with open(path, 'r', encoding='utf-8') as f:
                            return json.load(f)
                except Exception as e:
            ...
            def save_character_paths(self):
                ...
                try:
                    dir_path = os.path.join(os.getcwd(), 'JSON')
                    os.makedirs(dir_path, exist_ok=True)
                    path = os.path.join(dir_path, f'daoyan_character_paths_{self.node_id}.json')
                    with open(path, 'w', encoding='utf-8') as f:
            ...
            def _normalize_key(self, k):
                """Standardize key format: remove prefixes, strip, convert to int str if digit"""
                ...
                try:
                    ...
                    if os.path.exists(path):
                        ...
                        for k, v in raw_data.items():
                            ...
                            # Ensure list
                            videos = v if isinstance(v, list) else [v]
                            # Filter empty
                            videos = [x for x in videos if x]
                            
                            if norm_k in clean_data:
                                # Merge
                                for vid in videos:
                                    if vid not in clean_data[norm_k]:
                                        clean_data[norm_k].append(vid)
                                dirty = True
                            else:
                                clean_data[norm_k] = videos
                                # Mark dirty if key changed (e.g. shot_1 -> 1)
                                if norm_k != k:
                                    dirty = True
                        
                        if dirty:
                            print(f"[Director] Cleaned video paths: {len(raw_data)} -> {len(clean_data)} keys")
            ...
            class PropPromptWorker(QThread):
                ...
                def __init__(self, provider, model, api_key, api_url, hunyuan_api_url, image_path):
                    ...
                    if app:
                        if not hasattr(app, '_active_prop_prompt_workers_qpg'):
                            app._active_prop_prompt_workers_qpg = []
                        app._active_prop_prompt_workers_qpg.append(self)
                ...
                def _cleanup_worker(self):
                    ...
                    from PySide6.QtWidgets import QApplication
                    app = QApplication.instance()
                    if app and hasattr(app, '_active_prop_prompt_workers_qpg'):
                        if self in app._active_prop_prompt_workers_qpg:
                            app._active_prop_prompt_workers_qpg.remove(self)
                    self.deleteLater()
                def run(self):
                    try:
            ...
            def read_google_script_data(self, google_node):
                ...
                for r, row_data in enumerate(rows):
                    ...
                    if current_shot and current_shot in self.video_paths:
                        cached_val = self.video_paths[current_shot]
                        video_list = []
                        video_path_to_show = ""
                        
                        if isinstance(cached_val, list):
                            video_list = cached_val
                            # 默认显示最后一个，或者检查是否有存储的偏好
                            if video_list:
                                video_path_to_show = video_list[-1]
                        else:
                            video_list = [cached_val]
                            video_path_to_show = cached_val
                            
                        if video_path_to_show and os.path.exists(video_path_to_show):
                            # 确保列存在 (至少9列)
                            ...
                            # 检查当前单元格是否已经有选择的视频
                            current_item = self.table.item(r, 8)
                            if current_item:
                                stored_path = current_item.data(Qt.UserRole)
                                if stored_path and stored_path in video_list and os.path.exists(stored_path):
                                    video_path_to_show = stored_path
                            
                            self.set_cinema_cell_ui(r, video_path_to_show, video_list=video_list)
                ...
                if hasattr(self, 'video_paths') and self.video_paths:
                    for r in range(row_count):
                        ...
                        if shot_num in self.video_paths:
                            cached_val = self.video_paths[shot_num]
                            video_list = []
                            video_path_to_show = ""
                            ...
                            if socket.data_type == DataType.VIDEO:
                                for connection in socket.connections:
                                    ...
                                    if hasattr(target_node, 'receive_data'):
                                        target_node.receive_data(f"PENDING:{shot_text.strip()}", DataType.VIDEO)
            ...
            def set_cinema_cell_ui(self, row_idx, video_path, video_list=None):
                """设置电影院单元格的 UI (不涉及保存)"""
                item = self.table.item(row_idx, 8)
                if not item:
                    item = QTableWidgetItem()
                ...
                if video_list is not None:
                    if existing_widget and hasattr(existing_widget, 'video_paths'):
                        if existing_widget.video_paths != video_list:
                            should_update = True
                    ...
                    if current_path != video_path or not existing_widget:
                        should_update = True
                
                if not should_update and existing_widget:
                    return
                ...
                video_paths_list = []
                if video_list:
                    video_paths_list = video_list
                else:
                    video_paths_list = [video_path] if video_path else []
                    item_shot = self.table.item(row_idx, 0)
                    if item_shot:
                        shot_num = item_shot.text().strip()
                        
                        keys_to_try = [shot_num]
                        if shot_num.startswith("shot_"):
                            keys_to_try.append(shot_num[5:])
                        else:
                            keys_to_try.append(f"shot_{shot_num}")
                        
                        merged_list = []
                        for k in keys_to_try:
                            if k in self.video_paths:
                                cached = self.video_paths[k]
                                if isinstance(cached, list):
                                    for v in cached:
                                        if v not in merged_list:
                                            merged_list.append(v)
                                elif cached and cached not in merged_list:
                                    merged_list.append(cached)
                        
                        if merged_list:
                            video_paths_list = merged_list
                        elif video_path:
                            video_paths_list = [video_path]
                
                if video_path and video_path not in video_paths_list:
                    video_paths_list.append(video_path)

                container = DraggableVideoWidget(video_paths_list, current_path=video_path)
            ...
            def clear_video_batch(self, rows):
                ...
                for r in rows:
                    ...
                    # Robust deletion using normalized key
                    target_norm = self._normalize_key(shot_number)
                    keys_to_delete = []
                    for k in self.video_paths.keys():
                        if self._normalize_key(k) == target_norm:
                            keys_to_delete.append(k)
                    
                    for k in keys_to_delete:
                        del self.video_paths[k]
            ...
            def update_cinema_cell(self, row_idx, video_path, shot_number=None, video_url=None, task_id=None):
                ...
                # 1. 确定 Row Index 和 Shot Number
                if row_idx < 0 or row_idx >= self.table.rowCount():
                    if shot_number:
                        target_norm = get_norm_key(shot_number)
                        for r in range(self.table.rowCount()):
                            item = self.table.item(r, 0)
                            if item and get_norm_key(item.text()) == target_norm:
                                row_idx = r
                                break
                
                if row_idx < 0 or row_idx >= self.table.rowCount():
                    print(f"[DEBUG] Invalid row {row_idx}")
                    return

                current_shot_num = shot_number
                if not current_shot_num:
                    item = self.table.item(row_idx, 0)
                    if item: current_shot_num = item.text()
                
                # 2. 保存数据 (统一归一化)
                primary_key = None
                if current_shot_num:
                    target_norm = get_norm_key(current_shot_num)
                    
                    # 寻找匹配键
                    matching_keys = []
                    for k in self.video_paths.keys():
                        if get_norm_key(k) == target_norm:
                            matching_keys.append(k)
                    
                    # 确定主键 (优先短的，即 "1" 优于 "shot_1")
                    if matching_keys:
                        matching_keys.sort(key=lambda x: (len(str(x)), str(x)))
                        primary_key = matching_keys[0]
                    else:
                        primary_key = str(current_shot_num).strip()
                        if primary_key.startswith("shot_"): primary_key = primary_key[5:]
                    
                    # 初始化主键
                    if primary_key not in self.video_paths:
                        self.video_paths[primary_key] = []
                    if not isinstance(self.video_paths[primary_key], list):
                        self.video_paths[primary_key] = [self.video_paths[primary_key]]
                    
                    # 合并数据
                    for k in matching_keys:
                        if k == primary_key: continue
                        old_data = self.video_paths[k]
                        if isinstance(old_data, list):
                            for v in old_data:
                                if v not in self.video_paths[primary_key]:
                                    self.video_paths[primary_key].append(v)
                        elif isinstance(old_data, str) and old_data not in self.video_paths[primary_key]:
                            self.video_paths[primary_key].append(old_data)
                    
                    # 追加新视频
                    if video_path and video_path not in self.video_paths[primary_key]:
                        self.video_paths[primary_key].append(video_path)
                ...
                video_list = []
                if primary_key and primary_key in self.video_paths:
                    video_list = self.video_paths[primary_key]
                elif video_path:
                    video_list = [video_path]
                
                self.set_cinema_cell_ui(row_idx, video_path, video_list=video_list)
                ...
                if video_url and task_id:
                    ...
                    if current_shot_num:
                        self.video_metadata[current_shot_num] = {'url': video_url, 'task_id': task_id}
                    ...
                    for socket in self.output_sockets:
                        if socket.data_type == DataType.VIDEO:
                            for connection in socket.connections:
                                target_node = connection.target_socket.parent_node
                                if hasattr(target_node, 'receive_data'):
                                    # Send normalized key if possible
                                    s_to_send = primary_key if primary_key else current_shot_num
                                    if s_to_send:
                                        target_node.receive_data(f"FINISHED:{s_to_send}", DataType.VIDEO)
                ...
                for r in range(row_count):
                    ...
                    if has_storyboard:
                        ...
                        if shot_item:
                            ...
                            # 可能是列表或单个路径
                            paths = self.video_paths[shot_num]
