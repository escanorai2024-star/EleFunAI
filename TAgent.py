...
def markdown_to_html(text):
    ...
    style_vars = {
        ...
        'base_text_color': '#e0e0e0',
        'primary_color': '#00ff88',
        'secondary_color': '#00bfff',
        'bg_color_main': '#0a0a0a',
        'bg_color_code': 'rgba(26, 26, 26, 0.95)',
        'border_color': '#2a2a2a',
    }
        ...
        if block.strip().startswith('```'):
            ...
            if code_match:
                ...
                html = (f'<div style="background-color: {style_vars["bg_color_code"]}; border-radius: 8px; margin: 16px 0; border: 1px solid {style_vars["border_color"]}; overflow: hidden;">'
                        f'<div style="padding: 8px 16px; background-color: rgba(0,0,0,0.3); color: #888; font-size: 12px; border-bottom: 1px solid {style_vars["border_color"]};">代码</div>'
                        f'<pre style="margin: 0; padding: 16px; overflow-x: auto;"><code style="font-family: \'Consolas\', \'Monaco\', monospace; color: {style_vars["primary_color"]}; font-size: 13px; white-space: pre-wrap; word-wrap: break-word;">{escaped_code}</code></pre>'
                        f'</div>')
...
class AIAgentPage(QWidget):
    ...
    def create_input_area(self):
        ...
        panel.setStyleSheet("background-color: #0a0a0a; border-right: 1px solid #1a1a1a;")
        ...
        # 模型配置区
        config_frame = QFrame()
        config_frame.setStyleSheet("background-color: #1a1a1a; border-radius: 6px; padding: 8px;")
        ...
        provider_label.setStyleSheet("color: #888888; font-size: 11px;")
    ...
    def create_result_area(self):
        ...
        scroll.setStyleSheet("""
            ...
            QScrollBar::handle:vertical {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 rgba(0, 191, 255, 0.6), stop:1 rgba(0, 255, 136, 0.6));
                border-radius: 5px;
                min-height: 30px;
            }
        """)
    ...
    def on_preheat(self):
        ...
        self.result_text.setHtml("""
            <div style="text-align: center; margin-top: 50px;">
                <h2 style='color: #00ff88;'>🔥 正在预热...</h2>
                <p style='color: #888888;'>AI工作模式初始化中，请稍候...</p>
            </div>
        """)
    ...
    def _execute_preheat_step(self):
        ...
        try:
            ...
            error_html = f'''
            <div style="padding: 12px; background-color: rgba(255, 68, 68, 0.1); border-left: 3px solid #ff4444; border-radius: 4px;">
                <div style="color: #ff4444;">❌ 读取配置失败: {escape_html(str(e))}</div>
            </div>
            '''
    ...
    def _update_preheat_display(self, message):
        ...
        html = f"""
            <div style='text-align: center; margin-top: 100px;'>
                <h2 style='color: #00ff88;'>🔥 预热中...</h2>
                
                <div style='margin: 40px auto; width: 60%;'>
                    <div style='background-color: #1a1a1a; height: 30px; border-radius: 15px; overflow: hidden;'>
                        <div style='background: linear-gradient(90deg, #00ff88, #00bfff); height: 100%; width: {progress}%; transition: width 0.5s;'></div>
                    </div>
                ...
                <p style='color: #00ff88; margin-top: 30px; font-size: 14px;'>
                    {message}
                </p>
    ...
    def _on_preheat_error(self, error):
        ...
        error_html = f'''
        <div style="text-align: center; margin-top: 50px;">
            <div style="padding: 20px; background-color: rgba(255, 68, 68, 0.1); border-left: 3px solid #ff4444; border-radius: 8px; max-width: 500px; margin: 0 auto;">
                <h3 style="color: #ff4444; margin-bottom: 10px;">❌ 预热失败</h3>
                <p style="color: #e0e0e0;">预热过程在第 {self.preheat_step + 1} 步失败！</p>
                <p style="color: #888888; font-size: 13px; margin-top: 10px;">错误: {escape_html(str(error))}</p>
                <p style="color: #666666; font-size: 12px; margin-top: 15px;">请检查API配置后重试</p>
            </div>
        </div>
        '''
        self.result_text.setHtml(error_html)
    ...
    def _on_preheat_complete(self):
        ...
        self.result_text.setHtml("""
            <div style='text-align: center; margin-top: 100px;'>
                <h2 style='color: #00ff88;'>✅ 预热完成！</h2>
                <p style='color: #666; margin-top: 20px;'>AI已初始化完成，可以开始使用了</p>
                <p style='color: #00bfff; font-size: 14px; margin-top: 30px;'>
                    💡 提示：现在可以开始对话了
                </p>
            </div>
        """)
