...
class StoryboardPanel(QWidget):
    ...
    def _add_frame_thumb(self, img_path: Path, row: int, col: int, is_first: bool = False, is_last: bool = False):
        row_item = QFrame()
        ...
        self.grid.addWidget(row_item, row, col)
    ...
    def _render_frames(self):
        ...
        for i, p in enumerate(frames):
            self._add_frame_thumb(p, r, c, is_first=(i == 0), is_last=(i == total - 1))
            c += 1
            if c >= self._columns:
                c = 0
                r += 1
