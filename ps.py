...
Photoshop 集成模块
提供与 Photoshop 的桥接功能：
- 发送当前图层到 Photoshop 编辑
- 监听 Photoshop 保存，自动回传到画板
"""
...
class PhotoshopIntegration:
    ...
    def __init__(self, photo_page):
        ...
        # print('[Photoshop] 集成已初始化', flush=True)
    
    def send_to_photoshop(self):
        """发送当前选中图层到 Photoshop 编辑"""
        try:
            # 获取 Photoshop 程序路径
            ps_exe = self._get_photoshop_exe()
            if not ps_exe:
                # print('[Photoshop] 未配置 Photoshop 程序路径', flush=True)
                return
            
            # 获取当前选中图层
            canvas = getattr(self.photo_page, 'canvas', None)
            if not canvas:
                # print('[Photoshop] 无法访问画布', flush=True)
                return
            ...
            if lid is None:
                # print('[Photoshop] 未选择图层', flush=True)
                QMessageBox.warning(
                    self.photo_page, 
                    '提示', 
                    '请先选择一个图层\n\n如果没有图层，请先生成或添加图片到画板'
            ...
            if pix is None or pix.isNull():
                # print('[Photoshop] 无法获取图层内容', flush=True)
                QMessageBox.warning(self.photo_page, '提示', '选中的图层内容为空')
                return
            
            # 保存到桥接文件
            bridge_path = self.temp_dir / 'bridge_ps.png'
            if not pix.save(str(bridge_path), 'PNG'):
                # print('[Photoshop] 保存桥接文件失败', flush=True)
                QMessageBox.warning(self.photo_page, '错误', '无法保存临时文件')
                return
            
            # print(f'[Photoshop] 已保存桥接文件: {bridge_path}', flush=True)
            # print(f'[Photoshop] 图层ID: {lid}, 尺寸: {pix.width()}x{pix.height()}', flush=True)
            ...
            # print(f'[Photoshop] 已添加文件监听: {bridge_path}', flush=True)
            
            # 启动 Photoshop
            try:
                subprocess.Popen([ps_exe, str(bridge_path)], shell=False)
                # print(f'[Photoshop] ✓ 已启动 Photoshop: {ps_exe}', flush=True)
                # print(f'[Photoshop] 💡 请在 Photoshop 中编辑图片，保存后将自动回传到画板', flush=True)
            except Exception as e:
                # print(f'[Photoshop] ❌ 启动 Photoshop 失败: {e}', flush=True)
                QMessageBox.critical(self.photo_page, '错误', f'无法启动 Photoshop：\n{str(e)}')
        
        except Exception as e:
            # print(f'[Photoshop] 发送到 Photoshop 失败: {e}', flush=True)
            import traceback
            traceback.print_exc()
    
    def _get_photoshop_exe(self) -> str | None:
        """获取 Photoshop 程序路径（从设置或手动选择）"""
        ...
        if reply == QMessageBox.Yes:
            ...
            if path:
                ...
                # print(f'[Photoshop] 已保存程序路径: {path}', flush=True)
                return path
        
        return None
    
    def _on_bridge_file_changed(self, changed_path: str):
        """桥接文件变化时的回调（Photoshop 保存了文件）"""
        try:
            bridge_info = self._bridge_map.get(changed_path, {})
            if not bridge_info:
                # print(f'[Photoshop] 文件变化但无桥接信息: {changed_path}', flush=True)
                return
            
            origin = bridge_info.get('origin', 'canvas')
            lid = bridge_info.get('lid', None)
            
            # print(f'[Photoshop] 检测到文件变化: {changed_path}', flush=True)
            # print(f'[Photoshop] 来源: {origin}, 图层ID: {lid}', flush=True)
            
            # 重新加入监听（Photoshop 保存会替换文件导致监听失效）
            ...
            # print(f'[Photoshop] 文件变化处理失败: {e}', flush=True)
            import traceback
            traceback.print_exc()
    ...
    def _re_watch(self, path: str):
        ...
        try:
            if self._bridge_watcher and path not in self._bridge_watcher.files():
                ...
                # print(f'[Photoshop] 重新添加文件监听: {path}', flush=True)
        except Exception:
            pass
    ...
    def _schedule_load_bridge(self, path: str, origin: str, lid: int, attempts: int = 5, delay_ms: int = 300):
        ...
        def try_once():
            ...
            try:
                ...
                pix = QPixmap(path)
                if not pix.isNull():
                    # print(f'[Photoshop] ✓ 成功加载桥接文件: {pix.width()}x{pix.height()}', flush=True)
                    ...
                    # print(f'[Photoshop] ⚠️  加载的图片为空', flush=True)
                    pass
            
            except Exception as e:
                # print(f'[Photoshop] 加载失败 (剩余重试: {attempts-1}): {e}', flush=True)
                pass
            ...
            if attempts > 0:
                ...
                # print(f'[Photoshop] ❌ 加载失败，已达最大重试次数', flush=True)
                pass
    ...
    def _update_layer(self, lid: int, pix: QPixmap):
        ...
        try:
            ...
            try:
                ...
                # print(f'[Photoshop] 更新画布图层失败: {e}', flush=True)
                pass
            
            if not ok:
                # print(f'[Photoshop] ⚠️  画布图层更新失败，将添加为新图层', flush=True)
                self._add_new_layer(pix)
                return
            
            # print(f'[Photoshop] ✓ 已更新画布图层 ID={lid}', flush=True)
            ...
            try:
                ...
                if layers_panel and hasattr(layers_panel, 'list'):
                    ...
                    for i in range(cnt):
                        ...
                        if it and it.data(Qt.UserRole) == lid:
                            ...
                            # print(f'[Photoshop] ✓ 已更新图层面板缩略图 (索引={i})', flush=True)
                            break
                ...
                # print(f'[Photoshop] 更新图层面板缩略图失败: {e}', flush=True)
                pass
            
            # print(f'[Photoshop] ✅ 图层更新完成！', flush=True)
        
        except Exception as e:
            # print(f'[Photoshop] 更新图层失败: {e}', flush=True)
            import traceback
            traceback.print_exc()
    ...
    def _add_new_layer(self, pix: QPixmap):
        ...
        try:
            layers_panel = getattr(self.photo_page, 'layers_panel', None)
            if not layers_panel:
                # print('[Photoshop] 无法访问图层面板', flush=True)
                return
            
            # 添加新图层
            layers_panel.add_layer_pixmap(pix, '来自PS', sync_to_canvas=True)
            # print(f'[Photoshop] ✓ 已添加新图层: 来自PS ({pix.width()}x{pix.height()})', flush=True)
        
        except Exception as e:
            # print(f'[Photoshop] 添加新图层失败: {e}', flush=True)
            import traceback
            traceback.print_exc()
